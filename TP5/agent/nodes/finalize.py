...
def finalize(state: AgentState) -> AgentState:
    ...
    if intent == "reply":
        ...
        if cits:
            ...
            state.final_text = state.draft_v1.strip() or "Merci pour votre message. Je reviens vers vous dès que possible."

    elif intent == "ask_clarification":
        state.final_kind = "clarification"
        state.final_text = state.draft_v1.strip() or (
            "Bonjour, pourriez-vous préciser votre demande (contexte, module/projet) "
        ...
        state.actions.append({
            ...
            "summary": "Demande sensible nécessitant validation humaine.",
        ...
        state.final_text = "Message ignoré (hors périmètre)."
